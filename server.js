...
// configeration configz
const allowedPaths = new Set(['/vote', '/shop', '/my_projects', '/campfire', '/explore', '/votes/locked']);
const cache = new Map();
...
app.get('/slack/user/:id', async (req, res) => {
    ...
    try {
        const {data} = await axios.get(`https://slack.com/api/users.info?user=${slackId}`, {
            headers: { Authorization: `Bearer ${SLACK_BOT_TOKEN}` }
        });

        if (!data.ok) return res.status(500).json({error: 'Slack API Error', detail: data});
...
app.post('/fetch', async (req, res) => {
    ...
    try {
        ...
        if (requestedPath === '/explore') {
            const {data: rawData} = await axios.get('https://somps.vercel.app/api/projects');
            const projects = Object.values(rawData);
            extractedData = {projectTitles: projects.map(p => p.title || p.name || 'Untitled'), rawProjects: projects};
        } else {
            const {data: html} = await axios.get(`https://summer.hackclub.com${requestedPath}`, {
                headers: {
                    'Cookie': cookie,
            ...
            if (requestedPath === '/campfire') {
                extractedData = {
                    todayCodingTime: $('[data-hackatime-dashboard-target="todayTime"]').text().trim(),
                    totalCodingTime: $('[data-hackatime-dashboard-target="totalTime"]').text().trim()
                };
                ...
                $('a[href^="/projects/"]').each((i, el) => {
                    ...
                    projects.push({
                        ...
                        codingTime: $el.find('p.text-gray-400').html()?.split('<br>')[0]?.trim(),
                        devlogCount: $el.find('p.text-gray-400').html()?.split('<br>')[1]?.trim()
                    });
                });
                ...
                $('.card-with-gradient').each((i, el) => {
                    ...
                    shopItems.push({
                        ...
                        imageUrl: $card.find('img.rounded-lg.w-full.h-auto.object-scale-down.aspect-square.max-h-48').attr('src')?.trim() || null,
                        purchaseUrl: $content.find('form.button_to').attr('action')?.trim() || null,
                ...
                $('.card-with-gradient.text-lg.text-center').each((i, el) => {
                    ...
                    const $content = $card.find('.card-content');
                    votes.push({
                       approved: $content.find('p.text-2xl').text().trim().split('/')[0].trim(),
                       certified: Number(
                            $content
                                .find('p')
                                .filter((i, el) =>
                                    $(el).text().includes('certified projects, any amount of coding time)')
                                )
                                .first()
                                .text()
                                .match(/\d+/)?.[0] || 0
...
app.use((req, res) => {
    ...
    if (req.accepts('html')) {
        ...
        return res.json({error: 'Not found', message: `The requested URL ${req.originalUrl} was not found.`});
    }
